import TaliaImg from '../assets/img/Talia.png'; // Asegúrate de que las rutas sean correctas
import FrancoImg from '../assets/img/franco.png';
import ValeImg from '../assets/img/vale.png';
import NoeImg from '../assets/img/Noe.png';
const integrantes = [
    {
      nombre: "Espejo Talia Selene Marianela",
      rol: "Diseño",
      email: "talia@email.com",
      frase: "Siempre aprendiendo algo nuevo.",
      github:"https://github.com/TaliaEspejo",
      img: TaliaImg
    },
    {
      nombre: "Arcibia Franco",
      rol: "Programador principal",
      email: "franco@email.com",
      frase: "La lógica es el camino.",
      github: "https://github.com/Franco-2023-20",
      img: FrancoImg
    },
    {
      nombre: "Madrigal Lucia Valentina",
      rol: "Programador",
      email: "vale@email.com",
      frase: "Si puedes imaginarlo, puedes programarlo.",
      github:"https://github.com/valenmadrigal",
      img: ValeImg
    },
    {
      nombre: "Mamani Noe Franco",
      rol: "Programador",
      email: "francinoe3@email.com",
      frase: "El detalle hace la diferencia.",
      github: "https://github.com/francozxy",
      img: NoeImg
    }
];export default integrantes;